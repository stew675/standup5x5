...
static const char	*search_order = "qjxzvfwkbgpmhydcntlruaeios";
...
static uint32_t gv1 = 0, apcv1 = 0;
static uint32_t gv2 = 0, apcv2 = 0;
...
build_remap()
{
	...
	gv1 |= remap[('x' & 0x1F)];
	gv1 |= remap[('y' & 0x1F)];
	// Determine adjusted popcount for vowel1
	apcv1 = __builtin_popcount(gv1) - 5;
	...
	gv2 |= remap[('s' & 0x1F)];
	gv2 |= remap[('t' & 0x1F)];
	// Determine adjusted popcount for vowel2
	apcv2 = __builtin_popcount(gv2) - 5;
} // build_remap
